...
const FloorEditorCanvas = dynamic(
  () => import('@/features/table-management/components/FloorEditorCanvas'),
  { 
    ssr: false,
    loading: () => (
      <div className="w-full h-96 bg-gray-100 rounded-lg border-2 border-dashed border-gray-300 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500">Please wait while the editor loads</p>
        </div>
      </div>
    )
  }
...
export default function TableManagementPage() {
  // Access all required store properties
  const store = useTableStore();
  const tables = store.tables || [];
  const zones = store.zones || [];
  const fixtures = store.fixtures || [];
  const selectedTableIds = store.selectedTableIds || [];
  const isDraftMode = store.isDraftMode ?? true;
  ...
  const viewMode = store.viewMode || '2D';
  const showGrid = store.showGrid ?? true;
  const showZones = store.showZones ?? false;
  const gridSize = store.gridSize || 20;
  const snapToGrid = store.snapToGrid ?? true;
  const hasOverlaps = store.hasOverlaps ?? false;
  const totalTables = store.totalTables || 0;
  const totalCapacity = store.totalCapacity || 0;
  const availableTables = store.availableTables || 0;
  const tableStates = store.tableStates || {};
  
  // Store actions
  const addTable = store.addTable;
  const undo = store.undo;
  const redo = store.redo;
  const saveDraft = store.saveDraft;
  const activateLayout = store.activateLayout;
  const loadDraft = store.loadDraft;
  const setViewMode = store.setViewMode;
  const toggleGrid = store.toggleGrid;
  const toggleZones = store.toggleZones;
  const setGridSize = store.setGridSize;
  const toggleSnapToGrid = store.toggleSnapToGrid;
  const clearSelection = store.clearSelection;
  const exportLayout = store.exportLayout;
  const importLayout = store.importLayout;
  const alignTables = store.alignTables;
  const distributeTablesEvenly = store.distributeTablesEvenly;
  
  console.log('TableManagementPage - Tables:', tables.length, 'isLoading:', isLoading);
  ...
  useEffect(() => {
    console.log('Checking tables initialization - Tables:', tables.length, 'isLoading:', isLoading);
    if (tables.length === 0 && !isLoading && addTable) {
      console.log('Adding sample tables...');
      // Add some sample tables to demonstrate the functionality
      addTable({ x: 120, y: 80, shape: 'round', capacity: 4 });
      addTable({ x: 280, y: 80, shape: 'square', capacity: 4 });
      addTable({ x: 440, y: 80, shape: 'rect', capacity: 6 });
    }
  }, [tables.length, isLoading, addTable]);
